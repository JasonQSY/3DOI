...
from monoarti import _resnet_scene as resnet
...
class UNet(nn.Module):
    def __init__(self, arch, pretrained, n_embedding):
        """
        n_embedding: size of the embedding
        """
        super(UNet, self).__init__()

        orig_resnet = resnet.__dict__[arch](pretrained=pretrained)
        ...
        # embedding
        self.embedding_conv = nn.Conv2d(channel, n_embedding, (1, 1), padding=0)
        #self.embedding_conv = nn.Conv2d(channel, 64, (1, 1), padding=0)
        # depth prediction
